...
type Pos struct {
	...
	TableBox        []TableBox       `gorm:"foreignKey:PosUUID;references:UUID"`
	Zones           []Zone           `gorm:"foreignKey:PosUUID;references:UUID"` // Liste des zones associées au point de vente
	Livraisons      []Livraison      `gorm:"foreignKey:PosUUID;references:UUID"` // Liste des livraisons associées au point de vente
}
